...
const debug = require('debug')('ewd-memory-globals:sequence');
const padIfInteger = require('../utils/padIfInteger');
const isNormalInteger = require('../utils/isNormalInteger');
...
function sequence(node, direction) {
  ...
  if (glob) {
    ...
    let parentSubs = subscripts.slice(0, -1);
    let seed = subscripts.slice(-1)[0];
    let seedAfter = padIfInteger.call(this, seed) || '';

    debug('parentSubs: %j', parentSubs);
    debug('seed: \'%s\'; after: \'%s\'', seed, seedAfter);
    seed = seedAfter;

    let key = 'children:' + glob;
    if (parentSubs.length > 0) {
      const parentSeed = parentSubs.slice(-1)[0];
      debug('parentSeed: \'%s\'', parentSeed);

      if (seed === '' && isNormalInteger(parentSeed)) {
        const parentSubsAfter = parentSubs.slice(0, -1);
        debug('parentSubs: \'%s\'; after: \'%s\'', parentSubs, parentSubsAfter);
        parentSubs = parentSubsAfter;
      }

      key = key + this.key_separator + flattenArray.call(this, parentSubs);
    }
